...
function SectionObj:NewButton(buttonText, buttonInfo, callback)
                        ...
                        function SectionObj:NewColorPicker(colorText, colorInfo, default, callback)
                                ...
                                Library.Flags[colorText] = default
                                ...
                                Info.Position = UDim2.new(0, 10, 0, 20)
                                Info.BackgroundTransparency = 1
                                Info.Text = colorInfo or "Click to change"
                                Info.TextColor3 = Color3.fromRGB(150, 150, 150)
                                Info.Font = Enum.Font.Gotham
                                Info.TextSize = 11
                                Info.TextXAlignment = Enum.TextXAlignment.Left
                                ...
                                local colors = {
                                    Color3.fromRGB(0, 137, 123), Color3.fromRGB(255, 0, 0),
                                ...
                                ColorBtn.MouseButton1Click:Connect(function()
                                    ...
                                    Library.Flags[colorText] = newColor
                                    callback(newColor)
                                end)
                        end
                        ...
                        function SectionObj:NewKeybind(keyText, keyInfo, default, callback)
                                ...
                                Info.Position = UDim2.new(0, 10, 0, 20)
                                Info.BackgroundTransparency = 1
                                Info.Text = keyInfo or "Press to bind"
                                Info.TextColor3 = Color3.fromRGB(150, 150, 150)
                                Info.Font = Enum.Font.Gotham
                                Info.TextSize = 11
                                Info.TextXAlignment = Enum.TextXAlignment.Left
                                ...
                                KeyBtn.MouseButton1Click:Connect(function()
                                    listening = true
                                    KeyBtn.Text = "..."
                                    KeyBtn.BackgroundColor3 = Color3.fromRGB(0, 137, 123)
                                end)

                                UserInputService.InputBegan:Connect(function(input, gp)
                                    if listening and input.UserInputType == Enum.UserInputType.Keyboard then
                                        listening = false
                                        Library.Flags[keyText] = input.KeyCode
                                        KeyBtn.Text = input.KeyCode.Name
                                        KeyBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
                                        callback(input.KeyCode)
                                    elseif not gp and input.KeyCode == Library.Flags[keyText] then
                                        callback(input.KeyCode)
                                    end
                                end)
                        end
...
function Library:Notify(title, content, duration)
    ...
    local ScreenGui = Players.LocalPlayer.PlayerGui:FindFirstChild("EliteGlassUI")
    ...
    Stroke.Color = Color3.fromRGB(0, 137, 123)
    Stroke.Thickness = 1
...
function Library:SaveConfig(name)
    ...
    Library:Notify("Config", "Saved config: " .. name, 2)
end
...
function Library:LoadConfig(name)
    ...
    if isfile(path) then
        ...
        Library:Notify("Config", "Loaded config: " .. name, 2)
        return true
    end
    return false
end
