...
local c = {
    ...
    TextFade = Color3.fromRGB(15, 15, 18),
    Accent = Color3.fromRGB(110, 140, 255),
    GlassTint = Color3.fromRGB(20, 20, 25),
    Toggle = {
        Enabled = Color3.fromRGB(110, 140, 255),
        Disabled = Color3.fromRGB(40, 40, 50),
        Circle = Color3.fromRGB(235, 235, 240)
    },
    Notification = {
        Background = Color3.fromRGB(18, 18, 23),
        Border = Color3.fromRGB(50, 50, 60),
        Timer = Color3.fromRGB(110, 140, 255)
    }
}
...
local function CreateInstance(className, properties)
    ...
    for property, value in pairs(properties) do
        if property ~= "Parent" then
            if property == "Image" and (className == "ImageLabel" or className == "ImageButton") and typeof(value) == "string" then
                local resolved, offset, size = ResolveIcon(value)
                instance.Image = resolved
                if offset then instance.ImageRectOffset = offset end
                if size then instance.ImageRectSize = size end
            else
...
local function CreateGlassEffect(parent, tintTransparency)
    local strokeOuter = CreateInstance("UIStroke", {
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
    ...
    local strokeInner = CreateInstance("UIStroke", {
        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
        Color = c.Border,
        Transparency = 0.15,
        Thickness = 1.2,
...
local function LoadLucideIcons()
    ...
    if success and result then
        ...
        warn("Acrylic: Failed to load Lucide icons, using fallback")
    end
    return LucideIcons
end
...
local function GetLucideIcon(name)
    ...
    name = string.lower(string.match(name, "^%s*(.-)%s*$") or "")
    local sized = icons["48px"]
    if sized and sized[name] then
        local r = sized[name]
        return {
            id = r[1],
...
local function ResolveIcon(icon)
    ...
    if typeof(icon) == "string" then
        if string.find(icon, "rbxassetid://") == 1 or string.find(icon, "rbxasset://") == 1 or string.find(icon, "http") == 1 then
            return icon, nil, nil
        end
        -- Try Lucide by name
        local lucide = GetLucideIcon(icon)
        if lucide then
            return "rbxassetid://" .. lucide.id, lucide.imageRectOffset, lucide.imageRectSize
        end
        -- Fallback: assume it's a direct asset name
        return icon, nil, nil
    end
    return "", nil, nil
end
...
local function MakeDraggable(frame, handle)
    ...
    local function OnGlobalInput(input)
        if input == dragInput and dragging and frame and frame.Parent then
            local delta = input.Position - dragStart
            local newPos = UDim2.new(
                startPos.X.Scale,
                math.clamp(startPos.X.Offset + delta.X, -frame.AbsoluteSize.X + 50, workspace.CurrentCamera.ViewportSize.X - 50),
                startPos.Y.Scale,
                math.clamp(startPos.Y.Offset + delta.Y, 0, workspace.CurrentCamera.ViewportSize.Y - 50)
            )
    ...
    table.insert(connections, ui.InputChanged:Connect(OnGlobalInput))
    
    local id = "drag_" .. tostring(frame:GetDebugId())
...
function AcrylicBlur:_Render(distance)
    ...
    self._connections.cframe = workspace.CurrentCamera:GetPropertyChangedSignal("CFrame"):Connect(Update)
    self._connections.viewport = workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(Update)
    self._connections.fov = workspace.CurrentCamera:GetPropertyChangedSignal("FieldOfView"):Connect(Update)
    self._connections.pos = self._frame:GetPropertyChangedSignal("AbsolutePosition"):Connect(OnChange)
    self._connections.size = self._frame:GetPropertyChangedSignal("AbsoluteSize"):Connect(OnChange)
    self._connections.render = rs.RenderStepped:Connect(Update)
    task.spawn(OnChange)
end
...
function Library:_CreateMainv0rtexd()
    ...
    CreateGlassEffect(self.container)
    
    -- Inner glass overlay
    local glassOverlay = CreateInstance("Frame", {
        Name = "GlassOverlay",
        BackgroundColor3 = c.GlassTint,
